...
class phAstellasPanelConcretJob(override val defaultArgs: pActionArgs) extends pActionTrait {
    ...
    override def perform(args: pActionArgs): pActionArgs = {
        ...
        val hosp_ID = args.asInstanceOf[MapArgs].get("hosp_ID").asInstanceOf[DFArgs].get
        ...
        val total = {
            ...
            val have_market_gycx = delete_double_gycx.join(markets_match, delete_double_gycx("GYC_MOLE_NAME") === markets_match("MOLE_NAME"))
                    ...
                    .withColumn("MARKET", replaceMarket(col("MKT")))
                    .drop("MKT")
        ...
        val product_match = {
            product_match_file
                    .withColumn("min0", replaceDelimiter(product_match_file("min1")))
                    .select("min0", "min2", "STANDARD_MOLE_NAME", "STANDARD_APP2_COD", "STANDARD_PRODUCT_NAME")
                    .distinct()
        }
        ...
        val deletedTotal = {
            modifiedMarketTotal.filter(!(col("MARKET") === "佩尔市场" && (col("STANDARD_APP2_COD") =!= "粉针剂" && col("STANDARD_APP2_COD") =!= "注射剂")))
                    ...
                    .filter(!(col("MARKET") === "阿洛刻市场" && col("STANDARD_APP2_COD") === "喷雾剂"))
                    .filter(!(col("MARKET") === "阿洛刻市场" && col("STANDARD_APP2_COD") === "汽雾剂"))
